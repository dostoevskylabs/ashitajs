...
class AshitaClient extends net.Socket {
  ...
  onData ( data ) {
    ...
    switch ( data.type ) {
      ...
      case "peerLocation":
        if ( data.content.requestorIds[data.content.requestorIds.length -1] === this.nodeManager.getNodeId ) {
          ...
          this.nodeManager.whoHasAnswer( data.content.peerId, newNodes, route );
        }
      break;
